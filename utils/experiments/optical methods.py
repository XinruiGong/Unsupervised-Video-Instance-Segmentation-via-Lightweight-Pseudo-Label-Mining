...
#     if img1 is None or img2 is None:
#         raise ValueError("Could not open or find the images!")
#     alg = cv2.optflow.createOptFlow_PCAFlow()
#     # Calculate optical flow
#     flow = alg.calc(img1, img2, None)
...
#     alg=cv2.optflow.createOptFlow_Farneback()
#     flow=alg.calc(img1,img2,None)
...
def calculate_corner_flows(flow, img_shape, corner_size=0.15):
    ...
    Return:
    A dictionary containing average flows for each corner.
    """
...
def process_and_save_corner_flows(base_input_folder, base_output_folder,corner_size=0.15):
    """
    ...
    for folder_name in os.listdir(base_input_folder):
        ...
        for i in range(len(image_files) - 1):
            ...
            average_corner_flows = calculate_corner_flows(flow, img1.shape,corner_size=0.15)
...
def main():
    base_input_folder = "waymo_subset_0.055\\png_image"
    base_output_folder = "waymo_subset_0.055\\optical_flow_DIS"
